...
const triggerAutomaticNotification = async ({ eventType, studentId, teacherId, message: fallbackMessage, adminId = null, data = {} }) => {
    try {
        ...
        const template = NotificationTemplate
            ? await NotificationTemplate.findOne({ eventType, isActive: true }).lean()
            : null;
        ...
        if (methods.includes('email')) {
            emailResult = await sendEmail({
                ...
                attachments,
                templateTitle: subjectEmail,
                notificationType: eventType,
                emailTheme: data.emailTheme || null,
                allowHtml: true,
                actionUrl: data.actionUrl || process.env.FRONTEND_URL || '',
                actionLabel: data.actionLabel || 'Open Portal'
            });
        }
